  ...
  this.init = function() {
    ...
    if (this.selectobj && this.selectobj.options) {
      for (var i=0; i < this.selectobj.options.length; i++) {
        ...
        this.optionscopy[i].text = this.selectobj.options[i].text;

        // Set the value for the Option.
        // If the value wasn't set in the original select list,
        // then use the text.
        if (this.selectobj.options[i].value) {
          this.optionscopy[i].value = this.selectobj.options[i].value;
        } else {
          this.optionscopy[i].value = this.selectobj.options[i].text;
        }
